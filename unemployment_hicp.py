...
3. Library pandas
4. Library datetime
5. Files: "eurostat-data_hicp.csv" and "eurostat-data_unemployment.csv" need to
   be in Your directory file

Author: Leszek Dekiert
Formatted with: Black 25.1.0

Quick analisys and data preperation for further future analisys
...
with open("eurostat-data_unemployment.csv") as unemployment_file:
...
with open("eurostat-data_hicp.csv") as hicp_file:
...
data_unemployment = {
    unemployment_country[0]: unemployment_country[1:],
...
df_unemployment = df_unemployment.sort_values(
    by=["Indicator_Unemployment"], inplace=False, ascending=False
)
print(df_unemployment[0:10])

df_unemployment = df_unemployment.sort_values(
    by=["Indicator_Unemployment"], inplace=False, ascending=True
)
print(df_unemployment[0:10])
...
df_hicp = df_hicp.sort_values(by=["Indicator_HICP"], inplace=False, ascending=False)
print(df_hicp[0:10])

df_hicp = df_hicp.sort_values(by=["Indicator_HICP"], inplace=False, ascending=True)
print(df_hicp[0:10])
...
drop_1 = full_table[
    (full_table.Country == "Euro area â€“ 20 countries " "(from 2023)")
].index
drop_2 = full_table[
    (full_table.Country == "European Union - 27 countries " "(from 2020)")
].index
